...
public class ProfileFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (user != null) {
            TextView tvFriendsCount = view.findViewById(R.id.tv_profile_friends_count);
            new com.tomaflow.app.data.repository.FriendRepository(user.getUid())
                    .getFriends()
                    .observe(getViewLifecycleOwner(), friends -> {
                        if (tvFriendsCount != null) {
                            tvFriendsCount.setText(String.valueOf(friends != null ? friends.size() : 0));
                        }
                    });
        }
        ...
        view.findViewById(R.id.btn_logout).setOnClickListener(v -> {
            new Thread(() -> {
                ...
                requireActivity().runOnUiThread(() -> {
                    // Clear Unread Badge Manager & Sign out on Main Thread
                    try {
